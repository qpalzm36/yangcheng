...
def _extract_answer_from_text(text: str) -> str:
    ...
    # 如果没有boxed，则使用GSM8K的 #### 格式提取答案
    answer_match = re.search(r'####\s*(.*)$', text, re.DOTALL)
...
# --- 配置 ---
def parse_args():
    parser = argparse.ArgumentParser(description="运行GSM8K推理脚本")
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsGSM/gsm8k_inference_log_llama3.jsonl", help="输出日志路径")
...
def create_content_prompt(problem: str, previous_step=None, retrieval_context: str = None) -> str:
    ...
    instruction = (
        ...
        "Use clear, concise language. At the end of your solution, include the final answer in \\boxed{answer} format inside the [CONCLUSION] section. "
        "For GSM8K, you can also use '#### final_answer' at the very end. "
...
def run_retrieval(query, retriever_model, faiss_index, knowledge_base, k=1):
    query_embedding = retriever_model.encode([query], show_progress_bar=False)
    query_embedding_np = query_embedding.astype(np.float32)
    ...
    request_states = [
        {
            ...
            "problem_data": {
                "problem": p["question"],
                "answer": p["answer"].split("####")[1].strip() if "####" in p["answer"] else None,
            },
            "generated_steps": [],
            "inference_trace": [],
            "finished": False
