...
# --- Configuration ---
BASE_MODEL_PATH = "/data/share_weight/Qwen2.5-Math-7B-Instruct"
INPUT_DATA_PATH = "/data/yangcheng/aaai/data/traindata/generatordata/generator_training_data_with_retrieval.jsonl"
OUTPUT_DIR = "/data/yangcheng/aaai/generator_finetuned/Qwen2.5-Math-7B-Instruct"
...
def create_content_prompt(problem: str, previous_step: Dict = None, retrieval_context: str = None) -> str:
    ...
    input_parts = [f'"problem": {problem_str}']
    if previous_step:
        # 关键修复：确保将上一步的字典通过json.dumps转换为字符串
        # 这与推理脚本的行为完全一致
        input_parts.append(f"\"previous_step\": {json.dumps(previous_step)}")
...
def process_data(input_data_path: str) -> Dataset:
    ...
    with open(input_data_path, 'r', encoding='utf-8') as f:
        for line in tqdm(f, desc="Parsing problems"):
            ...
            # Create a map from step number to its full key for efficient lookup
            step_map = {
            ...
            # Sort steps numerically to process them in order
            ...
            for step_num in sorted_step_nums:
                ...
                # --- 1. Find previous step (if it exists) ---
                previous_step_dict = None # 改为字典
                if step_num > 1:
                    prev_step_num = step_num - 1
                    if prev_step_num in step_map:
                        prev_key = step_map[prev_step_num]
                        # 关键修复：将上一步构建为字典，而不是字符串
                        previous_step_dict = {prev_key: data[prev_key]}

                # --- 2. Extract information from the current step ---
                ...
                # --- 3. Create Training Sample 1: Decision Learning ---
                # Goal: Learn to output <retrieval> or <no retrieval>
                
                # 为了与决策提示保持一致，我们需要一个previous_step的字符串版本
                previous_step_for_decision_str = None
                if previous_step_dict:
                    # 将字典转换回 "key": "value" 格式的字符串，供决策提示使用
                ...
                full_text1 = f"{prompt1}{decision_tag}<|im_end|>"
                training_texts.append(full_text1)

                # --- 4. Create Training Sample 2: Content Generation (Execution) ---
                # Goal: Learn to generate the step content, and an end-of-solution marker
                key_clean = key.replace(decision_tag, "").strip()

                # 关键修复 1: target_json_str 只包含干净的、有效的JSON
                ...
                # 关键修复 2: 将结束标记附加在完整的JSON对象之后
                assistant_output = target_json_str
                if is_solution_end:
                    # 结束标记现在位于JSON对象的外部
                    assistant_output += " [END_OF_SOLUTION]"

                full_text2 = f"{prompt2}{assistant_output}<|im_end|>"
...
# --- Main ---
def main():
    parser = argparse.ArgumentParser(description="微调Qwen2.5-Math-7B-Instruct生成器模型。")
    parser.add_argument("--base_model_path", type=str, default="/data/share_weight/Qwen2.5-Math-7B-Instruct", help="基础模型路径。")
    ...
    parser.add_argument("--output_dir", type=str, default="/data/yangcheng/aaai/generator_finetuned/Qwen2.5-Math-7B-Instruct", help="输出模型的保存目录。")
