...
def run_inference_vllm(generator_model_path, retriever_model_path, knowledge_base_docs_path, faiss_index_path, test_set_path, output_log_path):
    ...
    llm = LLM(
        ...
        gpu_memory_utilization=0.95,  # 降低显存使用率以避免OOM
        dtype="bfloat16",
        enforce_eager=True
    )
    ...
    request_states = [
        {
            "problem_id": i,
            "problem_data": {
                "problem": p["problem"],  # MATH-500中的问题文本
                "answer": p["answer"]     # MATH-500中的答案
            },
            "generated_steps": [],
            "inference_trace": [],
            "finished": False
...
def main():
    parser = argparse.ArgumentParser(description="运行MATH-500数据集推理任务。")
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsmath/inference_log_vllm_math500.jsonl", help="输出日志路径。")
