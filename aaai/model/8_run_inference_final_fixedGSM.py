...
def _extract_answer_from_text(text: str) -> str:
    ...
    # 尝试从GSM8K格式提取答案 (#### 后面的内容)
    gsm8k_match = re.search(r'####\s+(.*?)(?:\s*$|\n)', text, re.DOTALL)
    if gsm8k_match:
...
def create_content_prompt(problem: str, previous_step=None, retrieval_context: str = None) -> str:
    ...
    instruction = (
        ...
        "Use clear, concise language. When you reach the final answer, include it after '####' like this: '#### 42'. "
...
def main():
    parser = argparse.ArgumentParser(description="运行GSM8K数据集推理任务。")
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsGSM/inference_log_gsm8k.jsonl", help="输出日志路径。")
