...
def run_inference_vllm(args):
    ...
    llm = LLM(
        ...
        gpu_memory_utilization=0.3,  # 降低显存使用率
        dtype="bfloat16",
        enforce_eager=True
    )
    ...
    # 修改请求状态初始化，适应AIME数据集格式
    request_states = [
        {
            "problem_id": i,
            "problem_data": {
                "problem": p["question"],  # 使用 "question" 键
                "answer": p["answer"]     # AIME数据集中的答案
            },
            "generated_steps": [],
            "inference_trace": [],
            "finished": False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="运行Qwen7B AIME模型推理")
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsaime/inference_log_aimeqwen7b.jsonl", help="输出日志路径")
