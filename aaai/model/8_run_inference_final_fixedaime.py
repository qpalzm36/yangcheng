...
def run_inference_vllm(generator_model_path, retriever_model_path, knowledge_base_docs_path, faiss_index_path, test_set_path, output_log_path):
    ...
    for step in range(MAX_STEPS):
        ...
        for i, request in enumerate(tqdm(active_requests, desc="  - 处理生成内容")):
            ...
            # 从原始文本中移除结束标记（如果存在），得到需要被解析的纯JSON内容
...
def main():
    parser = argparse.ArgumentParser(description="运行AIME数据集推理任务。")
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsaime/inference_log_aimeqwen3B.jsonl", help="输出日志路径。")
