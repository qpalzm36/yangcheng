...
PROMPT_TEMPLATES = {
    "answer_accuracy": {
        "system": "You are a mathematical answer comparison expert for AIME (American Invitational Mathematics Examination) problems. Your task is to determine if an AI-generated answer (Generated Answer) is mathematically equivalent to the standard answer (Gold Answer). For AIME problems, the answer is always a positive integer. Return a JSON object containing only one key, 'is_correct', with a value of true or false.",
...
def main(args):
    ...
    for r in full_evaluation_data:
        ...
        if r["scores"]["final_answer_correct"] is True:
            year_stats[year]["correct"] += 1
    
    # 计算每年的准确率
    for year, stats in year_stats.items():
        if stats["total"] > 0:
            stats["accuracy"] = f"{stats['correct'] / stats['total']:.2%}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="运行Llama2 7B AIME评估脚本")
    parser.add_argument("--inference_log_path", type=str, default="/data/yangcheng/aaai/resultsaime/inference_log_aime_llama2.jsonl", help="推理日志文件路径")
    parser.add_argument("--evaluation_report_path", type=str, default="/data/yangcheng/aaai/resultsaime/evaluation_report_aimellama2.json", help="评估报告文件路径")
