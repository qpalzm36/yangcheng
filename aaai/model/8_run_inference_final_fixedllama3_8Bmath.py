...
def _extract_answer_from_text(text: str) -> str:
    ...
    # 如果上面都没匹配到，尝试提取最后一个等式的结果
    equation_match = re.findall(r'=\s*([-\d\.\/]+)', text)
    if equation_match: return equation_match[-1].strip()
    # 尝试直接提取数字作为答案
    number_match = re.search(r'(\d+)', text)
    if number_match: return number_match.group(1).strip()
    return None
...
# --- 配置 ---
def parse_args():
    parser = argparse.ArgumentParser(description="运行MATH-500推理脚本")
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsmath/inference_log_vllm_llama3_math500.jsonl", help="输出日志路径")
...
def run_inference_vllm():
    ...
    sampling_params_content = SamplingParams(
        max_tokens=MAX_NEW_TOKENS, temperature=0.1, stop=["<|end_of_text|>", "[END_OF_SOLUTION]"],
