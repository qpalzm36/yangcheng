...
def extract_final_answer_robust(generated_steps: list) -> str:
    ...
    if 'error' in last_step_obj and 'original_content' in last_step_obj:
        return extract_answer_from_gsm8k(last_step_obj['original_content'])
    ...
    return extract_answer_from_gsm8k(step_text)
...
def create_content_prompt(problem: str, previous_step=None, retrieval_context: str = None) -> str:
    ...
    instruction = (
        ...
        "Use clear, concise language. When you reach the final answer, include it in the CONCLUSION section and write '#### answer' after it. "
...
def run_inference_vllm(args):
    ...
    # 加载GSM8K数据集
    with open(args.test_set_path, 'r', encoding='utf-8') as f_in:
    ...
    for step in range(MAX_STEPS):
        ...
        decision_prompts = [
            create_decision_prompt(
                r["problem_data"]["question"], # GSM8K 使用 "question" 字段
        ...
        for i, request in enumerate(tqdm(active_requests, desc="  - 处理决策并准备执行")):
            ...
            if needs_retrieval:
                problem_text = request["problem_data"]["question"]
    ...
    with open(args.output_log_path, 'w', encoding='utf-8') as f_out:
        for request in tqdm(request_states, desc="写入日志文件"):
            ...
            gold_answer = None
            if "answer" in request["problem_data"]:
                gold_answer_text = request["problem_data"]["answer"]
                gold_answer = extract_answer_from_gsm8k(gold_answer_text)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="运行Qwen7B GSM8K模型推理")
    parser.add_argument("--generator_model_path", type=str, default="/data/yangcheng/aaai/generator_finetuned/Qwen2.5-Math-7B-Instruct", help="生成器模型路径")
    ...
    parser.add_argument("--test_set_path", type=str, default="/data/yangcheng/gsm8k/main/sampled.jsonl", help="测试集路径")
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsGSM/inference_log_gsm8k_qwen7b.jsonl", help="输出日志路径")
