...
def _extract_answer_from_text(text: str) -> str:
    ...
    if gsm8k_match: 
        # 提取数字答案（去除可能的单位和额外文本）
        answer_text = gsm8k_match.group(1).strip()
        # 尝试提取数字（包括负数、小数和分数）
        number_match = re.search(r'(-?\d+(?:\.\d+)?(?:/\d+)?)', answer_text)
        if number_match:
...
def parse_args():
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsGSM/inference_log_vllm_llama2_gsm8k.jsonl", help="输出日志路径")
...
def create_content_prompt(problem: str, previous_step=None, retrieval_context: str = None) -> str:
    ...
    system_instruction = (
        ...
        "Use clear, concise language. When you reach the final answer, include it in the CONCLUSION section with #### followed by the answer. "
