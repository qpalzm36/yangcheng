...
PROMPT_TEMPLATES = {
    "answer_accuracy": {
        "system": "You are a mathematical answer comparison expert specialized in AIME (American Invitational Mathematics Examination) problems. Your task is to determine if an AI-generated answer (Generated Answer) is mathematically equivalent to the standard answer (Gold Answer). Ignore differences in units, formatting, or trailing zeros. For example, '12' and '\\boxed{12}' are equivalent. Return a JSON object containing only one key, 'is_correct', with a value of true or false.",
...
def main(args):
    ...
    for result in tqdm(all_results, desc="评估结果"):
        problem_eval = {
            ...
            "generated_answer": result.get("final_generated_answer")
        }

        # 评估最终答案准确性
        if result.get("gold_answer") is not None and result.get("final_generated_answer") is not None:
    ...
    print("\n--- AIME 评估报告 ---")
    print(f"总评估问题数: {total_problems}")
    print(f"最终答案准确率: {report['final_answer_accuracy']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="运行Qwen7B AIME评估脚本")
    parser.add_argument("--inference_log_path", type=str, default="/data/yangcheng/aaai/resultsaime/inference_log_aimeqwen7b.jsonl", help="推理日志文件路径")
    parser.add_argument("--evaluation_report_path", type=str, default="/data/yangcheng/aaai/resultsaime/evaluation_report_aimeqwen7b.json", help="评估报告文件路径")
