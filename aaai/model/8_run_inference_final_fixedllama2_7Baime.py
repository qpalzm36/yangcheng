...
def _extract_answer_from_text(text: str) -> str:
    ...
    # Add specific pattern for AIME problems which often have numeric answers
    numeric_match = re.search(r'(?:answer|result|value)[^\d]*(\d+)', text, re.IGNORECASE)
    if numeric_match: return numeric_match.group(1).strip()
    return None
...
# --- 配置 ---
def parse_args():
    parser = argparse.ArgumentParser(description="运行AIME推理脚本")
    parser.add_argument("--generator_model_path", type=str, default="/data/yangcheng/aaai/generator_finetuned/Llama-2-7b-chat-hf", help="生成器模型路径")
    ...
    parser.add_argument("--test_set_path", type=str, default="/data/yangcheng/AIME/AIME_2020_2024_filtered.jsonl", help="测试集路径")
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsaime/inference_log_aime_llama2.jsonl", help="输出日志路径")
...
MAX_NEW_TOKENS = 1024  # Increased for more complex AIME problems
MAX_STEPS = 12
...
def create_content_prompt(problem: str, previous_step=None, retrieval_context: str = None) -> str:
    """为AIME问题创建提示"""
    system_instruction = (
        "You are an expert math solver specialized in AIME (American Invitational Mathematics Examination) problems. "
        "Your goal is to solve the given math problem step by step. "
        "Your response MUST be a single JSON object with a key like `\"Step N\"` and the explanation as the value. "
        "Use clear, concise language and rigorous mathematical reasoning. "
        "When you reach the final answer, include it in the CONCLUSION section with \\boxed{answer} format. "
        "Make sure the final answer is a positive integer as required by AIME. "
...
def run_inference_aime():
    ...
    with open(KNOWLEDGE_BASE_DOCS_PATH, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            text_parts = []
            for key in data.keys(): text_parts.append(f"{key}: {data[key]}")
            kb_docs.append("\n\n".join(text_parts))
    ...
    request_states = [
        {
            ...
            "problem_data": {"problem": p["question"], "answer": p["answer"]},
            "generated_steps": [],
            "inference_trace": [],
            "finished": False
