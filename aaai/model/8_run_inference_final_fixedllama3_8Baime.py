...
def parse_args():
    ...
    parser.add_argument("--output_log_path", type=str, default="/data/yangcheng/aaai/resultsaime/inference_log_vllm_llama3_8B_aime.jsonl", help="输出日志路径")
...
def run_inference_vllm():
    ...
    # 加载 AIME 测试集
    print(f"加载 AIME 测试集: {TEST_SET_PATH}")
    ...
    with open(OUTPUT_LOG_PATH, 'w', encoding='utf-8') as f_out:
        for request in tqdm(request_states, desc="写入日志文件"):
            ...
            # AIME 数据集没有 gold_steps，只记录问题和最终答案
            log_entry = {
