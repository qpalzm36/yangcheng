...
# 这是一个统一的自动化测试脚本，用于运行所有Qwen7B模型的推理任务。
#
# 使用方法:
# /data/yangcheng/run_inferenceqwen7B.sh [TASK_TYPE] [GENERATOR_MODEL_PATH] [RETRIEVER_MODEL_PATH] [KB_DOCS_PATH] [FAISS_INDEX_PATH] [TEST_SET_PATH] [OUTPUT_LOG_PATH]
#
# 参数说明:
#   TASK_TYPE: (必需) 任务类型，可选值: default, aime, gsm, math, all
#   GENERATOR_MODEL_PATH: (可选) 生成器模型路径。
#                         默认: /data/yangcheng/aaai/generator_finetuned_test/Qwen2.5-Math-7B-Instruct
...
# --- 配置 ---
DEFAULT_GENERATOR_MODEL_PATH="${BASE_DIR}/aaai/generator_finetuned/Qwen2.5-Math-7B-Instruct"
...
case "$TASK_TYPE" in
    "default")
        PYTHON_SCRIPT="${BASE_DIR}/aaai/model/8_run_inference_final_fixedqwen7B.py"
        DEFAULT_TEST_SET_PATH="${BASE_DIR}/aaai/test/test120/structured_test_set.jsonl"
        DEFAULT_OUTPUT_LOG_PATH="${BASE_DIR}/aaai/results/inference_log_test12_qwen7B.jsonl"
        ;;
    "aime")
        PYTHON_SCRIPT="${BASE_DIR}/aaai/model/8_run_inference_final_fixedqwen7Baime.py"
        DEFAULT_TEST_SET_PATH="${BASE_DIR}/AIME/AIME_2020_2024_filtered.jsonl"
        DEFAULT_OUTPUT_LOG_PATH="${BASE_DIR}/aaai/resultsaime/inference_log_aimeqwen7b.jsonl"
        ;;
    "gsm")
        PYTHON_SCRIPT="${BASE_DIR}/aaai/model/8_run_inference_final_fixedqwen7BGSM.py"
        DEFAULT_TEST_SET_PATH="${BASE_DIR}/gsm8k/main/sampled.jsonl"
        DEFAULT_OUTPUT_LOG_PATH="${BASE_DIR}/aaai/resultsGSM/inference_log_gsm8k_qwen7b.jsonl"
        ;;
    "math")
        PYTHON_SCRIPT="${BASE_DIR}/aaai/model/8_run_inference_final_fixedqwen7Bmath.py"
        DEFAULT_TEST_SET_PATH="${BASE_DIR}/MATH-500/test.jsonl"
        DEFAULT_OUTPUT_LOG_PATH="${BASE_DIR}/aaai/resultsmath/inference_log_math500_qwen7b.jsonl"
        ;;
    "all")
        echo "运行所有Qwen7B推理任务..."
        bash "$0" "default"
