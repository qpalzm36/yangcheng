...
# 这是一个自动化测试脚本，用于运行 Qwen2.5-Math-7B-Instruct 生成器模型的微调过程。
#
# 使用方法:
# /data/yangcheng/run_finetunegeneratorqwen7B.sh [BASE_MODEL_PATH] [INPUT_DATA_PATH] [OUTPUT_DIR]
#
# 参数说明:
#   BASE_MODEL_PATH: (可选) 基础模型路径。
#                    默认: /data/share_weight/Qwen2.5-Math-7B-Instruct
...
# --- 配置 ---
PYTHON_SCRIPT="${BASE_DIR}/aaai/model/7_finetune_generatorqwen7B.py"

# 参数的默认值
DEFAULT_BASE_MODEL_PATH="/data/share_weight/Qwen2.5-Math-7B-Instruct"
DEFAULT_INPUT_DATA_PATH="${BASE_DIR}/aaai/data/traindata/generatordata/generator_training_data_with_retrieval.jsonl"
DEFAULT_OUTPUT_DIR="${BASE_DIR}/aaai/generator_finetuned/Qwen2.5-Math-7B-Instruct"
...
# --- 执行微调 ---
echo "--- 开始微调 Qwen2.5-Math-7B-Instruct 生成器模型测试 ---"
